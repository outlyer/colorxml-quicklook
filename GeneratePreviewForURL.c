...
OSStatus GeneratePreviewForURL(void *thisInterface, QLPreviewRequestRef preview,
  ...
  requestedBundle =
      CFBundleGetBundleWithIdentifier(CFSTR("it.araneum.qlgenerator.xml"));
  ...
  printf("BUNDLEEEEEEEEEE  %s/%s", CFStringGetCStringPtr(bundlePath, 0),
         CFStringGetCStringPtr(resourcePath, 0));

  //  resultString = CFStringCreateWithCString(0, "<html>CAZZO C'HAI DA
  //  GUARDARE?</html>", 0);
  ...
  sprintf(tempfileURL,
          "file://localhost/tmp/it.araneum.qlgenerator.xml.%ld.html", tempo);

  sprintf(stringona,
          "xsltproc --nonet  --novalid \"/%s/%s/xmlverbatimwrapper.xsl\" "
          "\"%s\" > %s",
          CFStringGetCStringPtr(bundlePath, 0),
          CFStringGetCStringPtr(resourcePath, 0),
          CFStringGetCStringPtr(percorso, 0), tempfile);
  printf("\n%s\n", stringona);
  resultcode = system(stringona);

  printf("\nEXITCODE: %d\n", resultcode);

  //	percorso =  CFStringCreateWithFileSystemRepresentation(0,
  //CFStringGetCStringPtr(percorso, 0));

  //   data = CFStringCreateExternalRepresentation(NULL, percorso,
  ...
  if (resultcode == 0) {
    CFURLCreateDataAndPropertiesFromResource(NULL, // allocator
                                             sourceURL, &data, NULL,
                                             NULL, // properties
                                             &errorCode);
  } else {
  ...
  if (resultcode == 0)
    ...
    QLPreviewRequestSetDataRepresentation(preview, data, kUTTypePlainText,
                                          NULL);
